...
- 設定は`chrome.storage.local`へ保存し、`chrome.storage.sync`でバックグラウンド同期する。タブ操作はメモリ上の設定を使い、同期の完了を待たせない。アクティベーション履歴・タブスナップショット・復元タブの識別情報は`chrome.storage.session`を使用し、同期対象に含めない。短命な遷移ステートはメモリ内で扱う。
