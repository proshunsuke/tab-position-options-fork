...
export const handleTabActivated = async (activeInfo: { tabId: number; windowId: number }) => {
  ...
  // 復元時は一時的な先頭タブのactivationが、最終選択より先に届く。
  // 初期化snapshotですでに非アクティブなら古い通知なので、復元先の保護を消費しない。
  if (
    shouldInitialize &&
    isSessionRestoreTab(activeInfo.tabId) &&
    getActiveTabSnapshot(activeInfo.windowId)?.id !== activeInfo.tabId
  ) {
    consumeNewTabActivation(activeInfo.windowId, activeInfo.tabId);
    return;
  }

  const isRestoreActivation = consumeSessionRestoreActivation(
    activeInfo.windowId,
    activeInfo.tabId,
  );
